...
//     a minimum display time are done, then activates the new scene.
//     The loading screen's visuals (background/text) are static, set directly on the
//     objects in this scene - not per-stage.
// JP: 「Loading」シーンに配置する。SceneChangeManagerから遷移先のシーン名を取得し、
//     バックグラウンドで非同期に読み込みを行う。読み込み完了と最低表示時間の両方が満たされたら、
//     新しいシーンをアクティブにする。
//     ローディング画面の見た目（背景・テキスト）は、このシーン内のオブジェクトに直接設定された
//     固定のものであり、ステージごとには変化しない。
