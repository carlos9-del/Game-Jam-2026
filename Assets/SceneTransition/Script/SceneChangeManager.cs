...
public class SceneChangeManager : SingletonMonoBehaviour<SceneChangeManager>
{
    ...
    {
        // EN: Subscribe to Unity's global scene-loaded event. This is more reliable than calling
        //     code after a scene finishes loading, since a coroutine's host object can be destroyed
        //     mid-flight when its own scene (e.g. Loading) unloads.
        // JP: Unityのグローバルなシーン読み込み完了イベントを購読する。
        //     シーンが読み込まれた後にコードを直接呼び出す方法より確実である。
        //     コルーチンを実行しているオブジェクト自身が、そのシーン（例：Loading）のアンロードと共に
        //     途中で破棄されてしまう場合があるため。
        SceneManager.sceneLoaded += OnSceneLoaded;
    }

    // EN: Called automatically whenever any scene finishes loading.
    // JP: 何らかのシーンの読み込みが完了するたびに自動的に呼び出される。
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        // EN: Ignore the Loading scene itself - wait for the real target scene to load instead.
        // JP: 「Loading」シーン自体は無視し、実際の目的シーンが読み込まれるのを待つ。
    ...
    //     OnSceneLoaded (subscribed to SceneManager.sceneLoaded) handles the rest once it's active.
    // JP: 目的のシーンへ直接遷移する（Loadingシーンを経由しない）。
    //     シーンがアクティブになった後の処理は、SceneManager.sceneLoadedを購読しているOnSceneLoadedが行う。
    ...
    //     Once the real target scene loads, OnSceneLoaded picks it up automatically.
    // JP: 「Loading」シーンへ遷移する。実際の目的シーンの読み込みはLoadingシーン側で行う。
    //     実際の目的シーンが読み込まれると、OnSceneLoadedが自動的にそれを検知する。
