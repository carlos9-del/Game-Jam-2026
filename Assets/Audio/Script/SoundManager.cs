...
public class SoundManager : SingletonMonoBehaviour<SoundManager>
{
    ...
    // EN: Checks whether the given clip is already the one currently playing as BGM.
    //     Used to avoid restarting BGM that should continue seamlessly across scenes.
    // JP: 指定されたクリップが、現在BGMとして再生中のものと同じかどうかを確認する。
    //     シーンをまたいでシームレスに続くべきBGMを、再起動しないようにするために使用する。
    public bool IsBGMPlaying(AudioClip clip)
    {
        return bgmAudioSource.isPlaying && bgmAudioSource.clip == clip;
    }

    // EN: Stops any currently playing sound effect(s). / JP: 現在再生中のSEを停止する。
    public void StopSE()
    {
        seAudioSource.Stop();
    }
